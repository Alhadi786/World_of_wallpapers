...
public class GDPR {
    ...
    public static void updateConsentStatus(final Activity act) {
        ...
        if(config.getAdmob_app_id () != null) {
            consentInformation.requestConsentInfoUpdate ( new String[]{act.getString ( Integer.parseInt ( config.getAdmob_app_id ( ) ) )}, new ConsentInfoUpdateListener ( ) {
                @Override
            ...
            Log.d ( "APP ID","np app id available" );
           // Toast.makeText ( Context,"no app id",Toast.LENGTH_LONG).show ();
        }
    }
